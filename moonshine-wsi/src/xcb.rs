...
/// Check whether `window` itself (not merely its toplevel) is fullscreen,
/// i.e. its geometry covers the root (screen) window within a 2px tolerance.
///
...
/// dialogs, or in-game settings overlays that are smaller than the screen)
/// fall back to normal XWayland. Bypassing those races the XWM on
/// window creation/destruction and breaks multi-window launchers, or creates
/// competing bypass surfaces that flicker.
///
/// NOTE: we check the window's *own* geometry, not its toplevel's. A settings
/// menu may be a child of the fullscreen game window; checking the toplevel
/// would incorrectly bypass the child too.
pub(crate) unsafe fn xcb_is_fullscreen_window(connection: *mut libc::c_void, window: u32) -> bool {
	let (_, _, ww, wh) = match xcb_get_window_rect(connection, window) {
		Some(r) => r,
		None => return false,
	};
	let (root, _, _) = match xcb_query_tree_window(connection, window) {
		Some(t) => t,
		None => return false,
	};
	...
	// Fullscreen if the window itself covers the root within 2px tolerance.
	(ww as i32 - rw as i32).abs() <= 2 && (wh as i32 - rh as i32).abs() <= 2
}
